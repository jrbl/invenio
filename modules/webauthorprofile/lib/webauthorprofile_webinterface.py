...
from invenio.config import CFG_WEBAUTHORPROFILE_BOX_WAIT_TIMEOUT_BEFORE_SERVING

# Box for which it is possible to decide if to wait loading before serving the page
# + Name : Principal name
# + NameVariants : Name variations box
# + Affiliations : Affiliations box
# + Papers : Papers box
# + FrequentKeywords : Frequent keywords box
# + FrequentCoAuthors : Frequent Co-Authors box
# + Citations : Citations box

#try:
#    splconf = CFG_WEBAUTHORPROFILE_BOX_WAIT_TIMEOUT_BEFORE_SERVING.split(',')
#except AttributeError:
#    splconf = []
#
#if 'Name' in splconf or 'NameVariants':
#    get_person_names_dicts = wait_with_timeout(get_person_names_dicts)
#if 'Affiliations' in splconf:
#    get_institute_pub_dict = wait_with_timeout(get_institute_pub_dict)
#if 'Papers' in splconf:
#    get_summarize_records = wait_with_timeout(get_summarize_records)
#if 'FrequentKeywords' in splconf:
#    get_kwtuples = wait_with_timeout(get_kwtuples)
#if 'FrequentCoAuthors' in splconf:
#    get_coauthors = wait_with_timeout(get_coauthors)
#if 'Citations' in splconf:
#    get_cited_by_list = wait_with_timeout(get_cited_by_list)
#del splconf
...
class WebAuthorPages(WebInterfaceDirectory):
    ...
    def index(self, req, form):
        ...
        metaheaderadd = '<script type="text/javascript" src="%s/js/webauthor.js"> </script>' % (CFG_SITE_URL)
        metaheaderadd += """
        <style> 
        .hidden {
            display: none;
        ...
        req.write(websearch_templates.tmpl_search_pagestart(ln=ln))
            ...
            #req.write("<!-- WebAuthorPages are Bibauthorid-powered !-->")
        val = self.create_authorpage_websearch(req, form, self.person_id, ln)
        if val:
            req.write(val)
        return page_end(req, 'hb', ln)
    ...
    def create_authorpage_websearch(self, req, form, person_id, ln='en'):
        ...
        venuetuples, venuetuplesStatus = get_venuetuples(person_id)
        if venuetuples:
            pass
            #vtuples = venuetuples[0:MAX_VENUE_LIST]
        else:
            vtuples = str(venuetuples)
        ...
        if form.has_key('jsondata'):
            ...
            req.write(webauthorprofile_templates.tmpl_author_page(req,
                                            ...
                                            ln, \
                                            beval))
        #req.write(summarize_records)


        #if not_complete:
            #req.write("\nCONTENT NOT COMPLETE, PLEASE RELOAD PAGE!!!\n")
            #req.write(str(eval))
