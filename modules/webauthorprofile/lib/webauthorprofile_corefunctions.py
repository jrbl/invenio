...
from invenio.webauthorprofile_dbapi import get_cached_element, precache_element, cache_element, \
    expire_cache_element, expire_all_cache_for_person, get_expired_person_ids
...
from invenio.search_engine import get_fieldvalues_alephseq_like
...
MAX_COLLAB_LIST = 250
MAX_KEYWORD_LIST = 250
MAX_VENUE_LIST = 250
MAX_COAUTHOR_LIST = 250
MAX_HEP_CHOICHES = 10
...
def _get_institute_pub_dict(recids, names_list, person_id):
    return {}
    ...
    try:
        ...
        cid = person_id
    recids = perform_request_search(p='author:%s' % str(cid))
    a = format_records(recids, 'HWAP')
...
def _get_person_names_dicts(person_id):
    '''
    returns a dict with longest name, normalized names variations and db names variations.
    @param person_id: int personid
...
def _get_veryfy_my_pubs_list_link(person_id):
    '''
...
def _get_cited_by_list(pubs, person_id):
    '''
    list of citations
    '''
...
def _get_kwtuples(pubs, person_id):
    '''
    Returns the keyword tuples for given personid.
    @param person_id: int person id
    '''
...
def _get_collabtuples(pubs, person_id):
    '''
    Returns the keyword tuples for given personid.
    @param person_id: int person id
    '''
...
def _get_coauthors(personid, collabs):
    ...
    if collabs:
        ...
        exclude_recs = perform_request_search(p=query)
    else:
        exclude_recs = None

    personids = get_coauthor_pids(personid, exclude_recs)
    if MAX_COAUTHOR_LIST > 0:
        personids = personids[0:MAX_COAUTHOR_LIST]
    ...
    for p in personids:
        ...
        #paps = len(perform_request_search(p="author:%s author:%s" % (cid, cn)))
        paps = p[1]
        if paps:
...
def  _get_rec_query(bibauthorid_data, authorname, db_names_dict, person_id):
    '''
    Returns query to find author's papers in search engine
    '''
    rec_query = ""
...
def _get_hepnames_data(bibauthorid_data, person_id):
    ...
    hepRecord = perform_request_search(cc='HepNames', p=cid)[:MAX_HEP_CHOICHES]
    ...
    if not hepRecord or len(hepRecord) > 1:
        ...
        additional_records = perform_request_search(cc='HepNames', p=query)[:MAX_HEP_CHOICHES]
...
def _compute_cache_for_person(person_id):
    start = time.time()
    _ = expire_all_cache_for_person(person_id)
