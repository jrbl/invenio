...
class WebSearchNearestTermsTest(unittest.TestCase):
    ...
    def test_nearest_terms_box_in_unsuccessful_simple_spires_query(self):
        """ websearch - nearest terms box for unsuccessful simple SPIRES query """
        self.assertEqual([],
                         test_web_page_content(CFG_SITE_URL + '/search?p=find+a+ellisz',
                                               expected_text="Nearest terms in any collection are",
                                               expected_link_target=CFG_SITE_URL+"/search?ln=en&p=find+a+ellis",
                                               expected_link_label='ellis'))
